...
+ 缩短故障窗口带来的业务损失下降
- NVMe 采购或实例溢价
- 缓存复制、监控、淘汰和故障回填的运维成本
```

因此，NVMe 更适合作为**高频启动模型和关键服务的分层缓存**，而不是要求所有 GPU 节点无差别配置最大容量、最高规格的盘。低频模型可以留在共享存储或按需回源；热点模型再根据缓存命中率和 RTO 下沉到本地盘。
...
1. 当前最长阶段到底是模型传输、权重转换、JIT 还是 Graph？
2. 业务真正要求的是新节点完整冷启动、滚动发布，还是故障切换 RTO？
3. 这项优化每月会被触发多少次，能节省多少 GPU·分钟和故障损失？
