...
> 本文创建于官方权重公开后的第二天，以 vLLM 和 SGLang 两条运行时路线共同验证 Qwen3.8-27B。两条路线都已经在测试集群的单张 NVIDIA L20 上完成 FP8 部署、正确性、AIBrix 接入、四组同口径短时性能基线、MTP 1/2/3 A/B，以及 32K/64K/128K 长上下文容量曲线；vLLM 还完成了 OpenWebUI 真实对话。Vision 仍是下一阶段，本文会明确区分“已实测”和“待实测”。
...
公开实例为了保留单卡并发，日常仍将运行时上下文限制为 32,768 token。为确认更长窗口是否真实可用，本轮临时将同一服务依次重发为 64K 和 128K candidate，完成 `31K+1K`、`63K+1K`、`127K+1K` 的 C=1 容量曲线后恢复 32K。实际权重的 `max_position_embeddings` 为 262,144；对应架构的官方模型卡也将 262K 定义为原生上下文，并建议显存不足时尽量维持至少 128K，以保留复杂任务的 Thinking 能力。128K 随机 Token 测试之外，两套引擎都通过了 10%/50%/90% 三位置 Needle 检索，详细结果见 9.2.2。
...
为了让后续 DeepSeek、GLM-5.2、Combined TP=8、P/D 双 TP=8 和 TCP/RDMA 复测使用同一套请求定义，仓库新增了[公共测试套件](https://github.com/runzhliu/aik8s/tree/main/examples/llm-long-context-pd-benchmark)。正式占用大规模 GPU 资源前，先用已经运行的 Qwen3.8-27B vLLM/SGLang 接口做了一次低负载验证：客户端统一为 vLLM 0.26.0，固定 128 输入、64 输出、C=1、一个 Warm-up、四个正式请求、相同 Tokenizer 与 Seed。
...
#### 9.2.2 单卡 L20 的 32K、64K、128K 上下文曲线

为了把“模型支持 128K”从配置声明变成可验证结果，本轮在同一张 L20、同一份 Qwen3.8-27B FP8 权重、TP=1 和 FP8 KV Cache 下，将 vLLM 与 SGLang 依次重发为 32K、64K、128K。每档只改变运行时最大上下文，并执行刚好填满窗口的 C=1 用例：

| 窗口 | 输入 | 最大输出 | 总上下文 |
| ---: | ---: | ---: | ---: |
| 32K | 31,744 | 1,024 | 32,768 |
| 64K | 64,512 | 1,024 | 65,536 |
| 128K | 130,048 | 1,024 | 131,072 |

这些数字的具体含义、`128/64` 与 `4096/128` 的区别，以及 token、字符和并发为何不能混用，见测试套件中的[具体例子](https://github.com/runzhliu/aik8s/blob/main/examples/llm-long-context-pd-benchmark/TOKENS_EXPLAINED.md)。三档统一使用同一个 vLLM 0.26.0 benchmark 客户端、`/v1/completions`、固定 Seed、固定长度随机输入、`temperature=0`、`ignore-eos`、1 次预热和 3 次正式请求。

128K candidate 均一次启动成功，没有 OOM。vLLM 加载权重约占 27.64 GiB，留出约 8.69 GiB KV Cache，可容纳 263,600 个 KV tokens；运行时给出的 131,072-token 理论最大并发为 2.01 倍。SGLang 加载权重约占 28.47 GB，另外为混合线性注意力分配约 4.78 GB Mamba State 和 5.54 GB FP8 KV Cache，可容纳 181,320 个 KV tokens；内部短请求 slot 上限从 8 自动降到 6。这些 slot 数不能解释成可并发运行 6 个 128K 请求。

随机 Token 容量探测结果如下。时间单位已在表头标出：

| Engine | 窗口 / 请求 | 成功/总数 | 实际输入 tokens | 实际输出 tokens | 输出 tok/s | p95 TTFT (s) | p95 TPOT (ms) | p95 E2EL (s) |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| vLLM | 32K / 31K+1K | 3/3 | 95,232 | 3,072 | 15.06 | 12.68 | 58.08 | 72.09 |
| SGLang | 32K / 31K+1K | 3/3 | 95,232 | 3,066 | 16.27 | 12.44 | 52.14 | 65.68 |
| vLLM | 64K / 63K+1K | 3/3 | 193,536 | 3,072 | 12.58 | 30.58 | 59.48 | 91.40 |
| SGLang | 64K / 63K+1K | 3/3 | 193,536 | 3,066 | 13.34 | 30.45 | 53.46 | 85.03 |
| vLLM | 128K / 127K+1K | 3/3 | 390,144 | 3,072 | 8.56 | 82.79 | 62.33 | 146.55 |
| SGLang | 128K / 127K+1K | 3/3 | 390,144 | 3,066 | 8.86 | 82.72 | 56.68 | 140.49 |

两条 TTFT 曲线都明显不是线性增长：

| Engine | 32K p95 TTFT | 64K p95 TTFT | 128K p95 TTFT | 64K / 32K | 128K / 64K | 128K / 32K |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| vLLM | 12.68 s | 30.58 s | 82.79 s | 2.41× | 2.71× | 6.53× |
| SGLang | 12.44 s | 30.45 s | 82.72 s | 2.45× | 2.72× | 6.65× |

同一窗口下，SGLang 相对 vLLM 的 p95 TTFT 只低约 0.08%–1.88%，说明本轮超长 Prefill 基本处于同一量级；p95 TPOT 低约 9.07%–10.23%，带动 p95 E2EL 低约 4.13%–8.90%。SGLang 总输出吞吐高约 3.50%–8.01%，但它每档实际返回 3,066 tokens，而 vLLM 为 3,072，不能忽略这个长度差异。

128K 还额外执行了 Needle 检索。Chat Template 计入后，服务端每条 Prompt 为 130,000 tokens；密钥分别放在文档 10%、50%、90% 位置，两套引擎都是 3/3 精确返回。这证明当前 128K 配置不只是能分配显存，也能从长文开头、中部和尾部取回目标信息。不过三条人工 Needle 不能替代真实长文、RAG 和代码仓库质量评测。

这组数据有四个必须保留的限制：

1. 每个 cell 只有 3 条正式请求，是容量与趋势探测，不足以形成生产级 p95/p99；
2. benchmark 的首条正式请求复用了预热 Prompt，三档的第一条 TTFT 都明显命中前缀缓存，表中使用 p95 代表另外两条冷请求，输出吞吐仍混合了冷热请求；
3. 两套运行时都提示 FP8 KV 缺少校准 Scale 并回退到 1.0，Needle 通过不等于所有任务质量无损；
4. vLLM 使用文本模式，SGLang 加载完整多模态模型且禁用了 Prefill CUDA Graph，因此结果是当前可运行配置的实测，不是引擎普遍排名。

结论是：单卡 L20 确实能运行 Qwen3.8-27B FP8 的 128K C=1 请求，但 127K 输入的冷请求首 token 约需 83 秒。若业务大多数请求远低于 32K，日常保持 32K 更有利于并发和资源利用；只有确实需要长文档、长会话或仓库级上下文时，才值得按路由单独提供 64K/128K candidate。测试结束后，两套公开实例均已恢复为 32,768-token 配置。
