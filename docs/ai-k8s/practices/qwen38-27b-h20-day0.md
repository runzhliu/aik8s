---
title: Qwen3.8-27B 发布次日：vLLM × SGLang 的 L20/H20 0-day 部署与压测
description: 记录 Qwen3.8-27B 官方 FP8 制品、vLLM 与 SGLang 镜像、单卡 L20 Kubernetes 实测、OpenWebUI 接入，以及后续 H20、MTP 和长上下文 A/B
status: draft
last_reviewed: 2026-08-15
---

# Qwen3.8-27B 发布次日：vLLM × SGLang 的 L20/H20 0-day 部署与压测

> 本文创建于官方权重公开后的第二天，以 vLLM 和 SGLang 两条运行时路线共同验证 Qwen3.8-27B。两条路线都已经在测试集群的单张 NVIDIA L20 上完成 FP8 部署、正确性、AIBrix 接入和四组同口径短时性能基线；vLLM 还完成了 OpenWebUI 真实对话。H20、MTP、Vision 与长上下文仍是下一阶段，本文会明确区分“已实测”和“待实测”。
...
Opus 4.5 于 2025 年 11 月 24 日发布。Opus 4.6 于 2026 年 2 月 5 日发布，时间点在当年春节前夕，因此“春节前夕发布”比“春节发布”更准确。几个月后，一个开放权重的 27B Dense 模型被官方报告放进了与 Opus 4.6 相同的 coding benchmark 表格，甚至在部分项目上领先，这件事本身就足够炸裂。

但“部分榜单领先”不能简化成“所有编码能力全面超过”。这篇 0-day 实验真正要回答的是：**Qwen3.8-27B 能否先在一张 L20 上跑通，再扩展到 H20，以可复现的成本提供接近前沿闭源模型的编码、Agent、工具调用和长上下文体验。**
...
- `cu129` 与现有 L20/H20 CUDA 12.9 实验链路一致，避免同时升级 CUDA 13；
...
它并不会神奇地减少首次下载的数据量，但本地 Layer Cache、可分段验证以及后续由内网服务端复制，能把家庭 VPN 的不稳定性从 staging → 生产链路中拿掉。本次 staging 已有绝大多数 Layer，因此 push 主要补齐 Manifest，实际很快完成。
...
## 4. L20 首轮结果与 H20 扩展路线

官方 Recipe 在 FP8 + 262K 场景给出的保守参考是 TP=4，但 0-day 最有价值的问题之一，是这个 27B FP8 是否能先塞进更常见的一张 48 GB L20。实测答案是可以：固定 TP=1、32K、text-only、FP8 KV、MTP Off 后，服务完整跑通。
...
| P4 | 官方 FP8 | 1×L20/H20 | 32K | 关闭 | 1/2/3 tokens | MTP 接受率与 Decode 收益 |
| P5 | 官方 FP8 | 1/2×H20 | 64K/128K/262K | 按需 | 按需 | 长上下文路径与 TP 成本 |
| P6 | 官方 BF16 | 1/2×H20 | 32K | 关闭 | 关闭 | FP8 正确性、显存和性能对照 |

如果使用 96 GB H20，FP8 TP=1 会得到明显更多 KV 与 Graph 余量；141 GB H20 则更适合继续推长上下文。两者都必须实测，不能把 L20 可启动直接外推成 H20 的吞吐或 262K 容量结论。

NVFP4 社区量化可以作为消费级或 Blackwell 路线补充，但 L20 属于 Ada、H20 属于 Hopper，不应把 NVFP4 作为本轮主基线。GGUF/llama.cpp 适合 Mac 或 CPU 做提示词与 Agent 接口冒烟，也不能替代数据中心 GPU 上的 vLLM/SGLang 吞吐测试。
...
1. **能否部署**：镜像、权重、架构、API 和 Kubernetes 生命周期是否正确；
2. **运行边界**：单卡/多卡、上下文、并发、显存和冷启动上限；
3. **是否值得**：在固定 Harness 下，coding/Agent 质量、吞吐、延迟和 GPU 成本是否达到预期。

它不能仅凭官方 Benchmark 宣称 Qwen3.8-27B 已经全面替代 Opus 4.6，也不能仅凭一次短压测给出生产容量。真正炸裂的不只是榜单数字，而是一个 27B 开放权重模型已经能在一张 L20 上跑通完整调用链，并且有机会在后续 H20 实验中，以可审计、可复现、可持续运行的方式接近那种曾经只有前沿闭源模型才能带来的 vibe coding 体验。
...
- [Claude Opus 4.5 发布公告，2025-11-24](https://www.anthropic.com/news/claude-opus-4-5)
- [Claude Opus 4.6 发布公告，2026-02-05](https://www.anthropic.com/news/claude-opus-4-6)
