...
到了 2026 年春节前，Opus 4.6 又发布了。几个月后，我看到 Qwen3.8-27B 的官方模型卡把这个 27B 开放权重模型和 Opus 4.6 放在同一张 Coding Benchmark 表里，有些项目甚至更高。我的第一反应确实是震惊：27B 的开放模型已经走到这里了吗？

但“部分榜单领先”不能简化成“所有编码能力全面超过”。这篇 0-day 实验先记录它在固定 L20 环境下使用 vLLM 与 SGLang 时的配置差异、接口行为和性能数据，再逐步扩展到 H20、Coding、Agent 与长上下文测试。
