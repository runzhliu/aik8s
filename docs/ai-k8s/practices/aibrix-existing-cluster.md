...
本篇主要记录 CPU mock 和控制面验证。真实 NVIDIA GPU、Ray 两机模型并行、NIXL P/D KV Cache 传输及八节点碎片 GPU 运行 235B FP8 模型的后续实测，见：[AIBrix 真实 GPU 实测：从两机推理到八节点碎片 GPU](aibrix-gpu-multinode-pd-production.md)。
