---
title: HarnessRouter 综述：从模型路由到 Agent Harness 基础设施
description: 解析 HarnessRouter、UHP、Cloud 与 Community Edition，说明 Agent Harness 统一接口、任务生命周期、安全边界、成本及其与 DSH 和 LLM Gateway 的关系
status: evolving
...
| [LLM API 中转器](../inference/llm-api-relay-gateway-overview.md) | 模型请求 | 统一 OpenAI/Anthropic 等模型 API、Key、计费和路由 |
...
- [大模型 API 中转器与 LLM Gateway 综述](../inference/llm-api-relay-gateway-overview.md)
