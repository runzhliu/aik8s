---
title: CubeSandbox Agent Adapter v0.5：强审计、可信任务与五客户端实测
description: 从执行意图持久化、失败闭锁和人工核对，理解 CubeSandbox Agent Adapter v0.5 的审计边界与部署实践
status: tested
...
![五种 Agent 共用 CubeSandbox Agent Adapter](../../assets/rag-agent/cubesandbox-agent-adapter-v05/architecture.png)
...
![CubeSandbox Agent Adapter v0.3 到 v0.5 的演进](../../assets/rag-agent/cubesandbox-agent-adapter-v05/evolution.png)
...
![本地 Code Agent 到生产资源的可信执行边界](../../assets/rag-agent/cubesandbox-agent-adapter-v05/trusted-execution-boundary.png)
...
![v0.5.0 强审计的持久化提交链路](../../assets/rag-agent/cubesandbox-agent-adapter-v05/durable-audit-flow.png)
...
![v0.5.0 真实 MicroVM 与四客户端验收](../../assets/rag-agent/cubesandbox-agent-adapter-v05/runtime-clients.png)

删除并重建 Adapter Pod 前后，权威库里的已提交事件保持 **498 → 498**；第二个写入进程尝试打开同一份在线日志时，被独占文件锁拒绝：

![Pod 重建后的持久审计验收](../../assets/rag-agent/cubesandbox-agent-adapter-v05/persistence-restart.png)
...
![故障闭锁、人工核对与隐私扫描](../../assets/rag-agent/cubesandbox-agent-adapter-v05/fail-closed-recovery.png)
...
![Claude Code 通过 MCP 完成 v0.5.0 可信任务](../../assets/rag-agent/cubesandbox-agent-adapter-v05/claude-code-trusted-task.png)
