...
def request(endpoint: str, model: str, prompt: str, max_tokens: int) -> dict[str, float | int | str]:
    payload = json.dumps(
        {
            "model": model,
            "prompt": prompt,
...
def main() -> None:
    ...
    target_prompt = shared_prefix + "\nTarget session: answer with one short sentence."

    cold = [request(args.endpoint, args.model, target_prompt, args.max_tokens)]
    summarize("target_cold", cold)

    warm = [
        request(args.endpoint, args.model, target_prompt, args.max_tokens)
    ...
    def fill(index: int) -> dict[str, float | int | str]:
        ...
        return request(args.endpoint, args.model, filler, 1)

    with concurrent.futures.ThreadPoolExecutor(max_workers=args.eviction_concurrency) as pool:
        pressure = list(pool.map(fill, range(args.eviction_prompts)))
    summarize("cache_pressure", pressure)

    replay = [request(args.endpoint, args.model, target_prompt, args.max_tokens)]
    summarize("target_after_pressure", replay)
