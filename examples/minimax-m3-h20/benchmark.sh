...
TOKENIZER="${TOKENIZER:-/models/MiniMax-M3/v1}"
ENGINE="${ENGINE:-vllm}"
...
[ "${BENCH_CLIENT}" = "vllm" ] || die "M3 comparison uses one shared vLLM client"

case "${STAGE}" in
  smoke|baseline|all) ;;
  *) die "STAGE must be smoke baseline or all" ;;
esac
...
if [ "${EXECUTE}" = "1" ]; then
  command -v vllm >/dev/null 2>&1 || die "vllm CLI is required for the shared benchmark client"
    ...
    while [ "${repeat}" -le "${repeats}" ]; do
      filename="${ENGINE}__${case_id}__r${repeat}.json"
        command=(
          ...
          --request-id-prefix "minimax-m3-${ENGINE}-${case_id}-r${repeat}-"
          --metadata
          "engine=${ENGINE}"
          "case_id=${case_id}"
          "stage=${case_stage}"
      ...
      if [ "${EXECUTE}" = "1" ]; then
        flush_cache
        [ ! -e "${RESULTS_DIR}/${filename}" ] || die "result exists; use a new attempt directory"
        command -v timeout >/dev/null 2>&1 || die "GNU timeout required in benchmark client"
        timeout --signal=TERM --kill-after=30s "${ROUND_TIMEOUT:-1800}" "${command[@]}"
        python3 "${SCRIPT_DIR}/validate_result.py" "${RESULTS_DIR}/${filename}" "${num_prompts}" "${output_tokens}"
        executed=$((executed + 1))
      else
...
if [ "${EXECUTE}" = "1" ] && [ "${skipped}" -gt 0 ]; then
  die "matrix incomplete: ${skipped} selected cases exceeded MAX_CONTEXT"
fi
