...
L20 48 GB 是这次完整 BF16 单卡部署的可用起点。1024 基线峰值分别是 32.8 GB 和 40.4 GB；进入 2K 与编辑矩阵后，两套引擎都接近 45 GB，只剩约 3 GB 卡面余量。生产环境不能用 1024 的数字推算 2K 容量，还要给批处理、CUDA Graph 和显存碎片留空间。
...
NSFW 包含艺术裸体、成人暗示、明确性内容、真人色情化编辑和未成年人内容，风险并不相同。Qwen-Image-2.1 的模型卡没有声明内置 Safety Checker；本地部署以后，输入审核、输出审核、真人授权和审计责任都由服务运营方承担。

![NSFW 发布前治理链](assets/qwen-image21-l20/nsfw-governance.png)

企业服务至少应直接拒绝未成年人性内容、真人色情化和无授权私密内容；同时对 Prompt 与参考图做输入审核，对生成图片做输出审核。批量接口还要配合实名账号、角色权限、速率限制和审计日志，生成内容通过审核后才能进入展示或下载链路。

这次没有生成露骨样例，也没有通过解除过滤来证明模型能力。对企业更有价值的验证，是确认阻断、审计、下架和删除流程真实可用。
